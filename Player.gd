...
func _process(delta):
	...
	self.position += Vector3(-SPEED * xinput, 0.0, -SPEED*yinput)
	self.position.x = clamp(self.position.x, -BORDER_WIDTH,BORDER_WIDTH)
	self.position.z = clamp(self.position.z, -BORDER_HEIGHT - BORDER_HEIGHT_OFFSET,BORDER_HEIGHT - BORDER_HEIGHT_OFFSET)	
